...
class AppState:
    def __init__(self):
        self.outlook_token_manager = None
        self.gmail_token_manager = None
        self.analysis_progress = {'status': 'idle', 'progress': 0, 'message': ''}
        self.last_analysis = None
...
class OutlookTokenManager:
    ...
    def _init_app(self):
        cache_file = CACHE_DIR / 'outlook_token_cache.bin'
        self.cache = msal.SerializableTokenCache()
        if cache_file.exists():
            self.cache.deserialize(cache_file.read_text())
    ...
    def get_auth_url(self):
        """Get device code flow for authentication"""
        flow = self.app.initiate_device_flow(scopes=CONFIG['outlook']['scopes'])
        if "user_code" not in flow:
            raise Exception(f"Failed to create auth flow: {flow}")
        return flow

    def complete_auth(self, flow):
        """Complete device code authentication"""
        result = self.app.acquire_token_by_device_flow(flow)
    ...
    def disconnect(self):
        """Disconnect user and clear tokens"""
        self.current_token = None
        self.token_expiry = None
        self.account_info = None
...
class GmailTokenManager:
    ...
    def get_auth_url(self):
        ...
        credentials_file = Path(CONFIG['gmail']['credentials_file'])
        if not credentials_file.exists():
    ...
    def complete_auth(self, auth_code):
        ...
        # Save credentials
        token_file = CACHE_DIR / 'gmail_token.pickle'
        with open(token_file, 'wb') as token:
            import pickle
            pickle.dump(self.credentials, token)
...
# === EMAIL ANALYSIS FUNCTIONS ===
def fetch_outlook_messages(token_manager, max_items=1000, progress_callback=None):
    """Fetch messages from Outlook"""
    messages = []
    base_url = "https://graph.microsoft.com/v1.0/me/messages"
    ...
    headers = {"Authorization": f"Bearer {token_manager.get_fresh_token()}"}
    params = {
        "$select": "from,receivedDateTime,hasAttachments,subject,internetMessageHeaders",
        "$orderby": "receivedDateTime desc",
        "$top": "50",
        "$expand": "singleValueExtendedProperties($filter=id eq 'Integer 0x0E08')"
    }
...
def fetch_gmail_messages(token_manager, max_items=1000, progress_callback=None):
    """Fetch messages from Gmail"""
    if not GOOGLE_AVAILABLE:
        return []
...
def serve_frontend():
    """Serve the main HTML file"""
    try:
        # Read the HTML file content (this would be the file we created)
        html_file = Path(__file__).parent / 'index.html'
        if html_file.exists():
            return html_file.read_text()
        else:
            # If no separate file, return embedded HTML
            return """
            <!DOCTYPE html>
            <html><head><title>Email Space Analyzer</title></head>
            <body>
            <h1>Email Space Analyzer</h1>
            <p>The web application should be in index.html</p>
            <p>Please ensure all files are in the same directory.</p>
            </body></html>
            """
    except Exception as e:
        return f"Error loading frontend: {e}", 500
...
def get_status():
    ...
    if app_state.outlook_token_manager:
        outlook_connected = app_state.outlook_token_manager.is_connected()
        if outlook_connected:
            outlook_account = app_state.outlook_token_manager.account_info
    
    if app_state.gmail_token_manager and GOOGLE_AVAILABLE:
        gmail_connected = app_state.gmail_token_manager.is_connected()
        if gmail_connected:
            gmail_account = app_state.gmail_token_manager.account_info
...
def connect_provider(provider):
    """Initiate connection to email provider"""
    try:
        if provider == 'outlook':
            if not app_state.outlook_token_manager:
                app_state.outlook_token_manager = OutlookTokenManager()
            
            flow = app_state.outlook_token_manager.get_auth_url()
            return jsonify({
                'success': True,
            ...
            if not app_state.gmail_token_manager:
                app_state.gmail_token_manager = GmailTokenManager()
            
            auth_data = app_state.gmail_token_manager.get_auth_url()
            return jsonify({
                'success': True,
...
@app.route('/api/auth-complete/<provider>', methods=['POST'])
def complete_auth(provider):
    """Complete authentication for provider"""
    try:
        data = request.get_json()
        ...
        if provider == 'outlook':
            ...
            # Create flow with device code
            flow = {'device_code': device_code}
            success = app_state.outlook_token_manager.complete_auth(flow)
            
            if success:
                return jsonify({'success': True, 'account': app_state.outlook_token_manager.account_info})
            else:
            ...
            success = app_state.gmail_token_manager.complete_auth(auth_code)
            
            if success:
                return jsonify({'success': True, 'account': app_state.gmail_token_manager.account_info})
            else:
                return jsonify({'success': False, 'error': 'Authentication failed'}), 400
        ...
        logger.error(f"Error completing auth for {provider}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/disconnect/<provider>', methods=['POST'])
def disconnect_provider(provider):
    """Disconnect from email provider"""
    try:
        if provider == 'outlook' and app_state.outlook_token_manager:
            app_state.outlook_token_manager.disconnect()
            return jsonify({'success': True})
        
        elif provider == 'gmail' and app_state.gmail_token_manager:
            app_state.gmail_token_manager.disconnect()
            return jsonify({'success': True})
        
        else:
            return jsonify({'success': False, 'error': 'Provider not connected'}), 400
...
def start_analysis():
    ...
    try:
        ...
        outlook_connected = (app_state.outlook_token_manager and 
                           app_state.outlook_token_manager.is_connected())
        gmail_connected = (app_state.gmail_token_manager and 
                         app_state.gmail_token_manager.is_connected())
        
        if not outlook_connected and not gmail_connected:
            return jsonify({'success': False, 'error': 'No email providers connected'}), 400
        
        # Start analysis in background thread
        thread = threading.Thread(target=run_analysis, args=(max_emails,))
        thread.daemon = True
        thread.start()
...
def run_analysis(max_emails):
    ...
    try:
        ...
        if app_state.outlook_token_manager and app_state.outlook_token_manager.is_connected():
            total_providers += 1
        if app_state.gmail_token_manager and app_state.gmail_token_manager.is_connected():
            total_providers += 1
        
        def update_progress(current, total, provider_name):
            provider_progress = (current / total) * 100 if total > 0 else 100
            overall_progress = ((completed_providers / total_providers) * 100 + 
                              (provider_progress / total_providers))
            app_state.analysis_progress.update({
                'progress': min(overall_progress, 95),  # Reserve 5% for final processing
                'message': f'Fetching {provider_name} emails... ({current}/{total})'
            })
        
        # Fetch Outlook messages
        if app_state.outlook_token_manager and app_state.outlook_token_manager.is_connected():
            app_state.analysis_progress['message'] = 'Connecting to Outlook...'
            
            def outlook_progress(current, total):
                update_progress(current, total, 'Outlook')
            
            outlook_messages = fetch_outlook_messages(
                app_state.outlook_token_manager, 
                max_emails, 
                outlook_progress
            )
            all_messages.extend(outlook_messages)
            ...
            logger.info(f"Fetched {len(outlook_messages)} Outlook messages")
        
        # Fetch Gmail messages
        if app_state.gmail_token_manager and app_state.gmail_token_manager.is_connected():
            app_state.analysis_progress['message'] = 'Connecting to Gmail...'
            
            def gmail_progress(current, total):
                update_progress(current, total, 'Gmail')
            
            gmail_messages = fetch_gmail_messages(
                app_state.gmail_token_manager, 
                max_emails, 
                gmail_progress
            )
            all_messages.extend(gmail_messages)
            ...
            logger.info(f"Fetched {len(gmail_messages)} Gmail messages")
        ...
        app_state.analysis_progress.update({
            'status': 'completed',
            'progress': 100,
            'message': f'Analysis complete! Processed {len(all_messages)} emails.'
        })
        
        logger.info(f"Analysis completed successfully. {len(all_messages)} emails processed.")
...
def main():
    ...
    # Initialize token managers with existing credentials if available
    try:
        app_state.outlook_token_manager = OutlookTokenManager()
        logger.info("Outlook token manager initialized")
    except Exception as e:
        logger.warning(f"Failed to initialize Outlook token manager: {e}")
    
    if GOOGLE_AVAILABLE:
        try:
            app_state.gmail_token_manager = GmailTokenManager()
            if app_state.gmail_token_manager.load_existing_credentials():
                logger.info("Gmail credentials loaded successfully")
            else:
                logger.info("No existing Gmail credentials found")
        except Exception as e:
            logger.warning(f"Failed to initialize Gmail token manager: {e}")
    ...
    print("1. The web interface will open automatically in your browser")
    print("2. Connect your email accounts (Outlook and/or Gmail)")
    print("3. Click 'Analyze My Emails' to start the analysis")
    print("4. View results in the dashboard")
    print("5. Export data for Power BI if needed")
    print("="*60)
    print("🛑 Press Ctrl+C to stop the server")
    print("="*60 + "\n")
